...
def build_export_dataframe(results: List[Dict], selected_route: Dict, context: Dict) -> pd.DataFrame:
    """Tạo bảng dữ liệu đầy đủ để xuất CSV/Excel sau khi tính toán."""
    export_rows = []
    generated_at = dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    for rank, item in enumerate(results, start=1):
        row = {
            "Thời gian xuất": generated_at,
            "Xếp hạng": rank,
            "Tuyến đã chọn": selected_route.get("name", ""),
            "Nguồn tuyến": selected_route.get("source", ""),
            "Khoảng cách tuyến (km)": round(float(selected_route.get("distance_km", 0) or 0), 2),
            "Thời gian tuyến gốc (phút)": round(float(selected_route.get("duration_min", 0) or 0), 1),
            "Loại hàng": context.get("cargo_type", ""),
            "Khối lượng hàng (kg)": context.get("weight_kg", ""),
            "Mức độ cấp bách": context.get("urgency", ""),
            "Giao thông": context.get("traffic", ""),
            "Thời tiết": context.get("weather", ""),
            "Ngập": context.get("flood", ""),
            "Mục tiêu tối ưu": context.get("priority", ""),
            "Giới hạn drone (km)": context.get("drone_limit", ""),
        }
        row.update(item)
...
def dataframe_to_excel_bytes(df: pd.DataFrame) -> bytes:
    """Chuyển DataFrame thành file Excel trong bộ nhớ để Streamlit tải xuống."""
    output = BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        df.to_excel(writer, index=False, sheet_name="Ket_qua_de_xuat")
...
if routes:
    ...
    display_columns = ["Phương tiện", "Điểm", "Chi phí (VNĐ)", "Thời gian (phút)", "Tải trọng tối đa (kg)", "CO₂ ước tính (kg)", "Lý do", "Cảnh báo"]
    st.dataframe(
        df[display_columns],
    ...
    export_df = build_export_dataframe(results, selected_route, export_context)
    csv_data = export_df.to_csv(index=False, encoding="utf-8-sig").encode("utf-8-sig")
    excel_data = dataframe_to_excel_bytes(export_df)

    st.markdown("### 5.1. Xuất dữ liệu")
    export_col1, export_col2 = st.columns(2)
    with export_col1:
        st.download_button(
            label="⬇️ Tải kết quả CSV",
            data=csv_data,
            file_name=f"ket_qua_de_xuat_{dt.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            mime="text/csv",
            use_container_width=True,
        )
    with export_col2:
        st.download_button(
            ...
            data=excel_data,
            file_name=f"ket_qua_de_xuat_{dt.datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
